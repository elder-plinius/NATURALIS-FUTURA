...
const CSS = readFileSync(join(process.cwd(), 'templates/style.css'), 'utf-8');
...
const html = [
  ...
  '  </div>',
  '  <div id="bottom-nav">',
  '    <button class="nav-btn active" data-view="map" onclick="switchView(\'map\')">🗺️ Map</button>',
  '    <button class="nav-btn" data-view="risk-matrix" onclick="switchView(\'risk-matrix\')">📊 Risk Matrix</button>',
  '    <button class="nav-btn" data-view="bestiary" onclick="switchView(\'bestiary\')">📖 Encyclopaedia</button>',
  '    <button class="nav-btn" data-view="compounds" onclick="switchView(\'compounds\')">⚡ Compounds</button>',
  '    <button class="nav-btn" data-view="observatory" onclick="switchView(\'observatory\')">📡 Observatory</button>',
  '    <button class="nav-btn" data-view="field-guide" onclick="switchView(\'field-guide\')">📕 Field Guide</button>',
  '    <button class="nav-btn" data-view="technical" onclick="switchView(\'technical\')">🔬 Technical</button>',
  '    <div class="nav-spacer"></div>',
  '    <button class="nav-toggle" id="hope-toggle" onclick="toggleHope()">✨ Hope</button>',
  '    <button class="nav-toggle" onclick="toggleSearch()">🔍 Search</button>',
  '    <button class="theme-toggle" id="theme-toggle" onclick="toggleTheme()">🌙 Dark</button>',
  '    <select id="view-mode" onchange="setViewMode(this.value)" style="padding:4px 8px;border-radius:6px;font-size:11px;border:1px solid var(--input-border);background:var(--parchment);color:var(--ink)">',
